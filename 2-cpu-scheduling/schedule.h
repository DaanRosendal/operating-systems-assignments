...
typedef enum { NEW_PROCESS_EVENT, TIME_EVENT, READY_EVENT, IO_EVENT, FINISH_EVENT } event_type;
...
// Reorder the queue based on the given comparator function
static void queue_reorder(student_pcb **queue, int (*comparator)(const void *, const void *)) {
    ...
    student_pcb **array = (student_pcb **)malloc(length * sizeof(student_pcb *));
    student_pcb *current = *queue;
    ...
    // Sort the array
    qsort(array, length, sizeof(student_pcb *), comparator);
