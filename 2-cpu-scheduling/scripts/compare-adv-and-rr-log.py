...
This script compares the priority scheduling algorithm with the round robin algorithm and outputs a
logarithmically spaced set of problem sizes and configurations.
"""
...
def run_advanced_simulation(aging_time_interval, aging_factor, p_value):
    command = ['./bin/advanced', '-c', '0.5', '-i', '0.5', '-m', '0.5', '-p', str(p_value)]
...
# Generate logarithmically spaced values between 100 and 40960
def generate_log_space_values(start, stop, num_values):
    return np.logspace(np.log10(start), np.log10(stop), num_values, dtype=int).tolist()
...
# Define the problem sizes between 100 and 40960
p_values = generate_log_space_values(100, 40960, 20)
